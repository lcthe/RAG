...
Day1-7 完成了 RAG 核心链路的原型验证：文档加载 → 分块 → 嵌入 → 检索 → 生成。但原型和可用的系统之间还有很大差距。Web App 的目标不是重做 RAG 逻辑，而是把「能跑」变成「好用」——解决原型阶段的几个核心痛点：

**1. 没有持久化**：对话记录、文档索引都在内存或临时文件里，服务重启就丢。  
**2. 没有 API**：所有功能靠命令行调用，没法集成到其他系统里。  
**3. 没有前端**：只有终端输出，非技术同学没法用。  
**4. 没有并发**：ChromaDB 单进程读写，多人同时用就有问题。  
**5. 没有运维**：没有日志、监控、配置管理，出问题全靠 print 找。  

下面从各个维度对比原型和 Web App 的具体差异：
